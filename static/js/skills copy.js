...
function createSkillElements(container, currentEl, skills, maxww = 0, maxhh = 0) {
    ...
    skills.forEach((skill, index) => {
        ...
        line1.style.left = containerRect.width/2 * 0.9 + x + "px";
        line1.style.top = currentEl.clientHeight - y + "px";

        // console.log(angle)

        const transformAngle = angle <= 180 ? (angle == 180 ? 90 : 70) : 110;
        ...
        const scale = Math.exp(angleDiff / 30) - 1;
        ...
        const t = sy - container.getBoundingClientRect().top + 20 + wrapperRect.width;
    ...
    if (!maxhh || !maxww) {
        ...
        setTimeout(() => {
            ...
            const xOffset = (containerRect.width ) / 2 - currentRect.left - currentRect.width/4;
